...
const ExpensesPage: React.FC<any> = ({ currentUser }) => {
  ...
  const getCount = async (email: string) => {
    let { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}api/finances/expenses/count`, { headers: { email }, withCredentials: true })
      ...
      <div className='h-[95%] w-[95%] flex flex-col gap-4 pt-8 bg-white  rounded-md px-8 py-4 text-xl font-bold'>
              ...
              <div className="overflow-x-hidden md:overflow-hidden border rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200 font-medium">
                    {!isLoading && userExpenses.sort((a: ExpenseRecord, b: ExpenseRecord) => {
                      return new Date(b.dateSpent).getTime() - new Date(a.dateSpent).getTime()
                    }).map((expense, index) => {
        ...
        <div className="w-full flex justify-center items-center">
          <div className="flex justify-center items-center gap-2 border border-black w-fit">
            <div className="font-extralight text-xs flex justify-center items-center pl-2 hover:cursor-pointer hover:opacity-75" onClick={() => pageHandler('decrease')}><ArrowLeftIcon className="h-6 w-6" /></div>
            <div className="border-l border-r border-black p-2">{page}</div>
            <div className="font-extralight text-xs flex justify-center items-center pr-2 hover:cursor-pointer hover:opacity-75" onClick={() => pageHandler('increase')}><ArrowRightIcon className="h-6 w-6" /></div>
          </div>
        </div>
        {modalActive && <AddItemForm getUserRecords={getUserRecords} currentUser={currentUser} activateModalHandler={activateModalHandler} getCount={getCount} />}
      </div>
    </div>)
}
