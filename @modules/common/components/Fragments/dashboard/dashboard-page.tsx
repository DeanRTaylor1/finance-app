...
const DashboardPage: React.FC<any> = ({ currentUser }) => {
  ...
  const headlineItems = [
    ...
    userData && { title: 'Average Daily Spend', value: `${getCurrencySymbol(userData.currency)}${numberWithCommas(getDailySpend(userData.total))}` }
  ].filter(Boolean).map((headline, index) => {
    return <DashBoardBox key={index} title={headline.title} value={headline.value} />
  })
  ...
  return (
    <div className="h-fit p-8 min-h-[calc(90vh)]  max-w-[1000px] flex flex-col gap-8 justify-start items-center w-[100%]">
      ...
      {userData && <Fragment>
        ...
        <div className="h-18 w-full bg-white p-4  mr-6 ml-6 border-t-8 border-blue-500 flex flex-col xl:flex-row justify-between items-center gap-4 font-bold border border-b-black border-r-black border-l-black shadow-md rounded-sm">
          <span className="flex gap-2 items-center w-full justify-center"> <h2 className="w-[30%]">From:</h2> <DatePickerComponent date={startDate} getDate={setStartDate} getUserData={getUserData} /></span>
          <span className="flex gap-2 items-center justify-center w-full "> <h2 className="w-[30%]">To:</h2> <DatePickerComponent date={endDate} getDate={setEndDate} getUserData={getUserData} /> </span>

        </div>
        <DashboardChartContainer>
          {userData && <DashboardChart expenses={userData.expenses} startDate={startDate} endDate={endDate} dailySpend={getDailySpend(userData.totalOutgoings)} />}
        </DashboardChartContainer>
        <DashboardChartContainer>
          {userData && <DashboardBarChartTags tagData={userData.outgoingsSum} />}
        </DashboardChartContainer>
